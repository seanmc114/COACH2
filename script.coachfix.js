...
// Pick with short-term memory to avoid obvious repetition (kids spot tiny banks fast).
function pickFresh(arr, memKey, keep=6){
  try{
    if(!state) return pick(arr);
    state._coachPickMem = state._coachPickMem || {};
    const mem = state._coachPickMem[memKey] || [];
    const pool = arr.filter(x=> !mem.includes(x));
    const choice = pick((pool.length>=Math.min(3,arr.length)) ? pool : arr);
    mem.push(choice);
    while(mem.length>keep) mem.shift();
    state._coachPickMem[memKey] = mem;
    return choice;
  }catch(_){
    return pick(arr);
  }
...
// Build a one-line coach exemplar that never copies the learner's exact wording.
function buildCoachOneLine(prompt, learnerAnswer, lang, focusTag){
  if(lang!=="es") return "—";
  const rub = levelRubric(state.level);
  const p = String(prompt||"").trim();
  const a = String(learnerAnswer||"").trim();

  // Start from a prompt-based model (never the learner).
  let base = "";
  try{
    base = String(buildSuggestionForItem(p, "", lang, rub, "detail")||"").trim();
  }catch(_){ base = ""; }
  if(!base) base = "Es interesante y me gusta bastante.";

  // Make it SNAPPY: keep first sentence only.
  base = base.split(/[.!?]/)[0].trim();
  if(!base) base = "Es interesante";
  base = base[0].toUpperCase()+base.slice(1);

  // Apply a single upgrade inside ONE sentence, depending on today's focus.
  const add = (frag)=>{
    const b = base.replace(/[.!?]+$/,"").trim();
    return (b + " " + frag).replace(/\s+/g," ").trim();
  };

  const needsBe = ()=> !/\b(soy|eres|es|somos|sois|son|estoy|estás|está|estamos|estáis|están)\b/i.test(base);
  const hasConn = ()=> /\b(porque|pero|además|aunque|cuando|entonces)\b/i.test(base);

  // Prompt-specific safe one-liners (high variety, always usable)
  const pLow = p.toLowerCase();
  const bank = [];
  if(/classroom|clase|aula/.test(pLow)) bank.push(
    "Mi aula es pequeña pero luminosa.",
    "Mi clase es tranquila y hay pósters en las paredes.",
    "Mi aula es moderna y tenemos una pizarra digital.",
    "Mi clase es ruidosa a veces, pero trabajamos bien.",
    "Mi aula es cómoda y tengo un buen sitio para estudiar."
  );
  if(/teacher|profesor|profe|maestr/.test(pLow)) bank.push(
    "Mi profesor es amable y explica muy bien.",
    "Mi profesora es exigente, pero es justa.",
    "El profe es simpático y nos ayuda cuando tenemos dudas.",
    "Mi profesor es divertido y la clase pasa rápido.",
    "Mi profesora es paciente y siempre nos anima."
  );
  if(/friend|amig[oa]/.test(pLow)) bank.push(
    "Mi mejor amigo es simpático y juega al fútbol.",
    "Mi amiga es trabajadora y siempre dice la verdad.",
    "Mi amigo es divertido, pero también es responsable.",
    "Mi amiga es tranquila y me ayuda cuando lo necesito.",
    "Mi mejor amigo es alto y muy deportista."
  );
  if(/town|pueblo|ciudad|barrio/.test(pLow)) bank.push(
    "Mi barrio es tranquilo y hay un parque cerca.",
    "Vivo en un pueblo pequeño, pero es muy agradable.",
    "Mi ciudad es animada y hay muchas tiendas.",
    "Mi barrio es seguro y los vecinos son simpáticos.",
    "Mi pueblo es bonito y está cerca del mar."
  );
  if(/school|colegio|instituto|escuela/.test(pLow)) bank.push(
    "Mi instituto es grande y tiene un patio enorme.",
    "Mi colegio es moderno y las clases son interesantes.",
    "Mi instituto es estricto, pero aprendemos mucho.",
    "Mi escuela es tranquila y hay muchas actividades.",
    "Mi instituto está cerca de mi casa y voy a pie."
  );
  if(/house|home|casa/.test(pLow)) bank.push(
    "Mi casa es bastante grande y tiene un jardín.",
    "Mi casa es pequeña, pero es muy cómoda.",
    "Vivo en un piso y la sala es luminosa.",
    "Mi casa es moderna y la cocina es grande.",
    "Mi casa está cerca del centro y me gusta mucho."
  );

  // Choose a base from bank sometimes, to increase variety.
  if(bank.length && Math.random()<0.6){
    base = pickFresh(bank, "coach_bank_"+String(focusTag||"x"), 6);
  }

  // Focus upgrades (still one sentence)
  if(focusTag==="missing_be"){
    if(needsBe()){
      // turn noun phrase into a sentence
      base = add("es muy importante para mí.").replace(/^Es\s+/,"Es ");
    }
  } else if(focusTag==="no_connector"){
    if(!hasConn()){
      base = add("porque me gusta mucho.");
    }
  } else if(focusTag==="too_short" || focusTag==="detail"){
    // add ONE extra detail without adding a second sentence
    if(!/\b(y|pero|porque|además)\b/i.test(base)){
      base = add("y además es muy interesante.");
    }else if(!hasConn()){
      base = add("porque es importante.");
    }
  } else if(focusTag==="word_order"){
    // Demonstrate a clean pattern rather than echoing learner structure
    if(/\bme\s+gusta\b/i.test(pLow) || /gusta/.test(pLow)){
      base = "Me gusta mi clase porque es interesante.";
    }
  }
  ...
  // Hard rule: never equal to learner answer (normalized).
  if(a && typeof sameSuggestion==="function" && sameSuggestion(base, a)){
    const alt = bank.length ? pickFresh(bank, "coach_bank_alt_"+String(focusTag||"x"), 8) : "En mi opinión es muy interesante porque me gusta.";
    base = alt;
    if(!/[.!?]$/.test(base)) base += ".";
    // still avoid equality
    if(typeof sameSuggestion==="function" && sameSuggestion(base, a)){
      base = "En mi opinión es bastante interesante porque me gusta mucho.";
    }
  }
...
function presentCoachModal(){
  try{
    ...
    const WIN_LINES = [
      "Good. Keep it. Then upgrade it.",
      "That would score. Now we sharpen it.",
      "Better. Now make it clean.",
      "You did enough. I want more control.",
      "Acceptable. Now make it impressive.",
      "Good. Don’t waste the momentum.",
      "You’re in the match. Stay disciplined.",
      "Solid. Now add one smart detail."
    ];
    const LOSE_LINES = [
      "That costs points. We remove it.",
      "Not today. Fix the leak and go again.",
      "No excuses. One focus. Then repeat.",
      "We don’t drift. We correct.",
      "Too messy. Clean it up.",
      "You want marks? Earn them. Reps.",
      "You’re close — but close isn’t the score.",
      "Reset. One fix. Then back in."
    ];

    const vibe = passed ? pickFresh(WIN_LINES, "vibe_win", 7) : pickFresh(LOSE_LINES, "vibe_lose", 7);

    const line2 = mustGym
      ? pickFresh([
          "Gym is required. Earn the exit. Then you come back sharper.",
          "Straight to gym. Fix it once and it stays fixed.",
          "Gym time. Not punishment — preparation.",
          "Tunnel time. Quick reps, then back to the match."
        ], "line2_must", 6)
      : pickFresh([
          "Gym is optional. Review feedback, then decide if you want reps.",
          "You can review first… or you can train and level up faster.",
          "Feedback is there. Gym is there. Your choice.",
          "You’re not stuck — you’re one drill away."
        ], "line2_opt", 6);

    const reasonMap = {
      spelling: [
        "Spelling/accents cost easy points. Clean them and everything reads smarter.",
        "Small spelling errors shout. Quiet them down.",
        "If it looks messy, it scores messy. Fix the surface."
      ],
      verb_form: [
        "Wrong verb form breaks the sentence. We fix the engine first.",
        "If the verb is wrong, the whole line leaks points.",
        "Verb form = control. Control = credit."
      ],
      verb_ending: [
        "Verb endings matter: yo como / él come. Make it automatic.",
        "Endings tell the story. Get them right and you sound fluent.",
        "One ending wrong and the sentence wobbles. Stabilise it."
      ],
      agreement: [
        "Agreement matters (alto/alta). It’s quick points when tight.",
        "Endings must match. Make them match and you jump up.",
        "Agreement errors are loud. We make them silent."
      ],
      articles_gender: [
        "Articles/gender are small, but they shout beginner when wrong. Tighten them up.",
        "El/la, un/una — easy wins when you lock them in.",
        "Gender slips are cheap points lost. Stop donating points."
      ],
      articles: [
        "Articles make Spanish sound natural (un/una, el/la).",
        "Articles are small — but markers see them instantly.",
        "Lock articles in and your writing looks controlled."
      ],
      word_order: [
        "Word order changes meaning. Get the pattern and it becomes easy.",
        "English order in Spanish is a giveaway. Swap the pattern.",
        "Fix the pattern once. Then it stays fixed."
      ],
      missing_be: [
        "Descriptions need ‘to be’ (ser/estar). Without it, the line collapses.",
        "No ‘to be’ = no complete description. Put it in.",
        "Ser/estar is the frame. Put the frame in."
      ],
      no_connector: [
        "One correct connector makes your writing flow and score higher.",
        "Join ideas cleanly and you look more advanced instantly.",
        "Connectors are glue. No glue = points fall off."
      ],
      too_short: [
        "Too short = not enough credit. Add one detail and you jump up.",
        "One extra detail turns a weak line into a scoring line.",
        "Short lines don’t show skill. Add one controlled detail."
      ],
      detail: [
        "Detail wins marks. One extra fact is the difference.",
        "One smart detail = instant upgrade.",
        "Say more — but cleanly."
      ]
    };
    ...
    const drillMap = {
      spelling: [
        ...
        "Drill: fix the surface fast.",
        "Drill: tidy the writing."
      ],
      verb_form: [
        "Drill: verb forms (soy/tengo/me gusta).",
        "Drill: pick the correct verb form.",
        "Drill: repair the verb."
      ],
      verb_ending: [
        "Drill: conjugate endings (yo como / él come).",
        "Drill: subject + ending.",
        "Drill: endings under pressure."
      ],
      agreement: [
        "Drill: adjective agreement (alto/alta).",
        "Drill: make endings match.",
        "Drill: agreement quick reps."
      ],
      articles_gender: [
        "Drill: articles + gender (el/la, un/una).",
        "Drill: el/la choices.",
        "Drill: gender lock-in."
      ],
      articles: [
        "Drill: articles (un/una, el/la).",
        "Drill: article choices.",
        "Drill: tidy articles."
      ],
      word_order: [
        "Drill: word order patterns (me gusta…).",
        "Drill: fix the pattern.",
        "Drill: rebuild the sentence shape."
      ],
      missing_be: [
        "Drill: ser/estar.",
        "Drill: add ‘to be’ fast.",
        "Drill: build the frame (ser/estar)."
      ],
      no_connector: [
        "Drill: connectors (y/pero/porque/además).",
        "Drill: choose the best connector.",
        "Drill: glue two ideas."
      ],
      too_short: [
        "Drill: add ONE extra detail.",
        "Drill: upgrade the line once.",
        "Drill: make it longer — cleanly."
      ],
      detail: [
        "Drill: add ONE extra detail.",
        "Drill: one upgrade only.",
        "Drill: add one smart detail."
      ]
    };
    ...
    const model = buildCoachOneLine(prompt, ans, state.lang, focusTag);
...
if(type==="detail" || type==="upgrade"){
        ...
        const base = tidySuggestion(String(model).split(".")[0] + "."); // never echo learner in choices
