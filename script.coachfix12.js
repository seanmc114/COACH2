...
if(t.includes("canteen") || t.includes("cafeter") || t.includes("cantina") || t.includes("comedor")){
  ...
  out = ["2 features (hair/eyes/height)", "1 connector", "1 extra detail"].concat(base);
} else if(t.includes("strength") || t.includes("weakness") || t.includes("puntos fuertes") || t.includes("debilidad") || t.includes("fortaleza")){
  out = ["one strength", "one weakness (pero…)", "one reason"].concat(base);
  ...
  out = ["place", "opinion", "why (porque)", "1 extra detail"].concat(base);
...
function jcMarking(promptText, answer, level, lang, rubric, det){
  ...
  // Score: encourage intelligible communication first, then reward extra features, then penalise real language cogs.
let score = 3; // baseline attempt credit
if(wc >= rubric.minWords) score = 5;               // clear, usable line
if(wc >= rubric.minWords + 3) score = 6;           // a bit of stretch

// Reward checklist coverage (but don't double-reward "Length")
const earnedBonus = earned.filter(l=>l!=="Length").length;
score += Math.min(4, earnedBonus); // up to 10 before deductions
