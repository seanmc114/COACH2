...
function detectTags(prompt, answer, lang, rubric){
  ...
  if(lang==="es"){
    ...
    // Noun + "eres" (common: treating a thing as "you are") → verb form / sentence structure
    if(/\b(cama|mesa|silla|cocina|clase|aula|habitaci[oó]n|casa|pueblo|ciudad|instituto|colegio)\s+(y\s+)?eres\b/.test(low)){
      tags.push("verb_form");
      examples.push("cama eres → la cama es / mi cama es");
    }
    // "y eres ..." with no clear subject before → likely wrong subject (should be es/está or add tú/yo)
    if(/\by\s+eres\b/.test(low) && !/\b(yo|t[uú]|tu|vosotros|nosotros|ellas|ellos|ella|[eé]l)\b[^.?!]{0,25}\beres\b/.test(low)){
      tags.push("verb_form");
      examples.push("eres needs a subject (tú eres) or change to es/está");
    }
    // "eres" used to describe a place/object (no 2nd-person context) → likely es/está
    if(/\bmi\s+(cama|casa|habitaci[oó]n|aula|clase|pueblo|ciudad)\s+eres\b/.test(low)){
      tags.push("verb_form");
      examples.push("mi X eres → mi X es/está");
    }
...
state.mark.items = state.mark.items.map(it=>{
  ...
  // Build a clean "Quick fix" line that NEVER parrots the learner.
  // (We already show "Your answer" above; the fix must be a fresh, safe line they can copy.)
  let fix = "";
  if(focusTag==="spelling" && it.answer){
    const fx = (state.lang==="es") ? wordFixesES(it.answer) : {fixed:String(it.answer||"")};
    fix = String(fx.fixed||"").trim();
  }
  // For everything else, use a prompt-based exemplar (no learner seeding)
  if(!fix || sameSuggestion(fix, it.answer)){
    fix = buildSuggestionForItem(it.prompt, "", state.lang, rubric, focusTag);
  }
  // Extra safety: if the fix still heavily overlaps with the learner's words, regenerate as a neutral model
  try{
    ...
    if(a && b){
      ...
      let hit=0;
      bt.forEach(t=>{ if(aset.has(t)) hit++; });
      const overlap = hit / Math.max(1, Math.min(at.length, bt.length));
      if(overlap > 0.55){
        fix = buildSuggestionForItem(it.prompt, "", state.lang, rubric, "detail");
      }
    }
  }catch(_){}
