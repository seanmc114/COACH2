...
function detectTags(prompt, answer, lang, rubric){
  ...
  // Prompt type (very lightweight, used for person mismatch + model selection)
  ...
  if(lang==="es"){
    ...
    // (This is a *big* leak and must beat "more detail".)
    ...
    if(describingOther){
      ...
      // Also catch "yo soy" etc. for best-friend/teacher
    ...
    // Common bad patterns (high value)
    ...
    // Missing to-be ONLY when truly absent
    if(!/\b(es|está|son|soy|eres|somos|estoy|estás|están)\b/.test(low) && /(es|está|son|soy|eres|somos|estoy|estás|están)/.test("es")){
      // (the second part keeps lint quiet; do nothing)
    ...
    // Agreement: very rough heuristic (only a few common cases)
...
function buildSuggestionForItem(prompt, answer, lang, rubric, focusTag){
  ...
  const BANK = {
    ...
    general: [
      "Me gusta porque es cómodo y bastante práctico.",
      "En general es bonito y me siento bien allí.",
      "Es interesante y además es bastante útil.",
      "Normalmente es tranquilo, pero a veces hay ruido.",
...
state.mark.items = state.mark.items.map(it=>{
  ...
  const priority = ["blank","gibberish","person_mismatch","verb_form","verb_ending","word_order","missing_be","articles_gender","agreement","articles","spelling","no_connector","too_short","detail"];
...
"${basePrompt||"die Aufgabe"}"`;
      ...
      if(el.wsHelp){
        ...
        const starter = (function(){
          ...
          return ["Empieza con: “Mi ____ es ____”", "Añade: porque ___", "Añade: y además ___"];
        })();
