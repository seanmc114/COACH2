...
function presentCoachModal(){
  try{
    ...
    const reasonMap = {
      blank: "No answer = no credit. Put ONE clean sentence down.",
      gibberish: "Notes aren’t a sentence. Make ONE clean line with a verb.",
      person_mismatch: "Wrong person: describe him/her, not ‘you’.",
    ...
    const drillMap = {
      blank: "Drill: one clean starter line.",
      gibberish: "Drill: turn notes into ONE sentence.",
      person_mismatch: "Drill: he/she forms (es/tiene/le gusta).",
    ...
    const html = `
      ...
      ${also.length ? `<div class="muted" style="margin-top:8px"><b>Also watch:</b> ${escapeHtml((Array.from(new Set(also))).join(" · "))}</div>` : ""}
...
function detectTags(prompt, answer, lang, rubric){
  ...
  // Basic empties
  if(!a || a==="—"){ tags.push("blank"); return {tags, examples}; }

  const pLow = p.toLowerCase();

  // Prompt type (lightweight, used for focus + model selection)
  const pType = (function(){
    if(/best\s+friend|amig[oa]\s+(?:mejor|favorit[oa])|tu\s+mejor\s+amig[oa]/.test(pLow)) return "best_friend";
    if(/teacher|profesor[ae]?|profe\b/.test(pLow)) return "teacher";
    if(/family|familia|padre|madre|mam[aá]|pap[aá]|herman[oa]|abuel[oa]/.test(pLow)) return "family";
    if(/bedroom|dormitorio|habitaci[oó]n/.test(pLow)) return "bedroom";
    if(/classroom|aula|clase/.test(pLow)) return "classroom";
    if(/favourite\s+place|favorite\s+place|lugar\s+favorit[oa]|sitio\s+favorit[oa]/.test(pLow) && /house|casa|hogar/.test(pLow)) return "house_place";
    if(/kitchen|cocina/.test(pLow)) return "kitchen";
    if(/bathroom|ba[nñ]o|aseo/.test(pLow)) return "bathroom";
    if(/living\s+room|sal[oó]n|comedor/.test(pLow)) return "living_room";
    if(/town|city|pueblo|ciudad|barrio/.test(pLow)) return "town";
    if(/school|instituto|colegio|escuela/.test(pLow)) return "school";
    if(/free\s+time|tiempo\s+libre|hobby|afici[oó]n|fin\s+de\s+semana/.test(pLow)) return "freetime";
    if(/yourself|describe\s+yourself|t[uú]\b\s+(?:mismo|misma)\b/.test(pLow)) return "self";
    if(/eat|drink|comer|beb(er|o)|desayun|almorz|cena|merienda|comida|bebida/.test(pLow)) return "food";
    return "general";
  })();

  // Coherence / gibberish detection (prevents "es 4. yo, hhermano..." passing as 'fine')
  const letters = (a.match(/[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]/g) || []).length;
  const nonSpace = a.replace(/\s+/g,"").length || 1;
  const letterRatio = letters / nonSpace;
  const digitCount = (a.match(/\d/g) || []).length;
  const weirdPunct = (a.match(/[{}[\]<>\\/_=+*]/g) || []).length;

  if(letterRatio < 0.55 || digitCount >= 2 || weirdPunct >= 2){
    tags.push("gibberish");
    examples.push("Make ONE clean sentence (not notes)");
  }

  // Too short (but only after we check for bigger issues)
  if(wc < (rubric?.minWords||6)) tags.push("too_short");

  if(lang==="es"){
    // Spelling/accents quick fixes (only when they look like real words)
    ...
    // Person mismatch: describing someone else but writing "you are / you have"
    const describingOther = (pType==="best_friend" || pType==="teacher" || pType==="family");
    if(describingOther){
      if(/\b(eres|estás|tienes|vas|juegas|hablas|comes|vives)\b/.test(low)){
        tags.push("person_mismatch");
        if(/\beres\b/.test(low)) examples.push("Use él/ella es (not eres)");
        else examples.push("Use he/she forms (not you-forms)");
      }
      if(/\b(soy|tengo|me\s+gusta|vivo)\b/.test(low) && !/\b(mi\s+mejor\s+amig|mi\s+profesor|mi\s+profe|mi\s+madre|mi\s+padre|mi\s+herman)/.test(low)){
        tags.push("person_mismatch");
        examples.push("Talk about them (he/she), not you");
      }
    }
    ...
    // Noun + "eres" (thing treated as "you are")
    if(/\b(cama|mesa|silla|cocina|clase|aula|habitaci[oó]n|casa|pueblo|ciudad|instituto|colegio|escuela)\s+y\s+eres\b/.test(low)){
      tags.push("verb_form");
      examples.push("Room/thing can't be 'eres' → use es/está");
    }
    if(/\b(cama|mesa|silla|cocina|clase|aula|habitaci[oó]n|casa|pueblo|ciudad|instituto|colegio|escuela)\s+eres\b/.test(low)){
      tags.push("verb_form");
      examples.push("Use es/está (not eres)");
    }

    // --- Missing MAIN verb (important): lists of nouns with no verb, especially for food/likes ---
    const hasCoreVerb =
      /\b(soy|eres|es|somos|son|estoy|estás|está|estamos|están|tengo|tienes|tiene|tenemos|tienen|hay|vivo|vives|vive|vivimos|viven|me\s+gusta|me\s+gustan|me\s+encanta|me\s+encantan|quiero|prefiero|suelo|normalmente|a\s+veces|juego|juegas|juega|jugamos|juegan|estudio|estudias|estudia|estudiamos|estudian|como|comes|come|comemos|comen|bebo|bebes|bebe|bebemos|beben|se\s+llama|mi\s+nombre\s+es)\b/.test(low);

    // If it's basically "nouns + y" and no verb, tag missing_verb
    const looksLikeNounList =
      !/[,.;:!?¿¡]/.test(low) && (/\by\b/.test(low) || wc<=4) && !/\b(porque|pero|adem[aá]s|cuando|aunque)\b/.test(low);

    if(!hasCoreVerb && (pType==="food" || looksLikeNounList)){
      tags.push("missing_verb");
      if(pType==="food") examples.push("Use: Me gusta(n) + comida");
      else examples.push("Add a verb (es/tengo/hay/me gusta…) to make a sentence");
    }

    // Missing to-be ONLY for descriptive prompts, and only if no stronger missing_verb tag
    const descriptivePrompt = ["self","best_friend","teacher","family","bedroom","classroom","town","school","house_place","kitchen","bathroom","living_room"].includes(pType);
    if(descriptivePrompt && !tags.includes("missing_verb")){
      if(!/\b(es|está|son|soy|eres|somos|estoy|estás|están)\b/.test(low)){
        tags.push("missing_be");
      }
    }

    // Connector missing: multiple clauses with no connector
    if(wc >= 8 && !/\b(y|pero|porque|adem[aá]s|cuando|aunque)\b/.test(low)){
      tags.push("no_connector");
    }

    // Agreement: rough heuristic
    if(/\b(habitaci[oó]n|casa|clase|aula)\s+es\s+\w+o\b/.test(low)){
      tags.push("agreement");
      examples.push("Feminine noun → adjective ends in -a");
    }

    // Verb ending: infinitive after subject (yo comer)
    if(/\b(yo|t[uú]|el|ella|nosotros|vosotros|ellos|ellas)\s+\w+(ar|er|ir)\b/.test(low)){
      tags.push("verb_ending");
      examples.push("Conjugate: yo hablo / él habla");
    }
  }
...
function focusLabel(tag, lang){
  const L = lang || "es";
  if(tag==="gibberish") return "One clean sentence";
  if(tag==="person_mismatch") return "Wrong person (you/he/she)";
  if(tag==="spelling") return (L==="es") ? "Spelling & accents" : "Spelling";
  if(tag==="verb_form") return "Verb forms";
  if(tag==="missing_verb") return "Missing main verb";
...
function tipForTags(tags, lang){
  ...
  if(lang==="es"){
    if(has("blank")) return "Put down ONE clean sentence. Then we build.";
    if(has("gibberish")) return "Turn notes into ONE sentence with a verb (es/tengo/hay/me gusta…).";
    if(has("person_mismatch")) return "Wrong person: describe him/her (él/ella es…), not ‘you are’.";
    if(has("verb_form")) return "Fix the verb form (soy/tengo/me gusta/es/está) — this is where marks vanish.";
    ...
    if(has("missing_verb")) return "Add a main verb (Me gusta(n)/Hay/Tengo/Es…) — not just a list.";
...
function pickRoundFocus(items, lang, rubric){
  ...
  const labelMap = {
    blank: "No answer",
    gibberish: "One clean sentence",
    person_mismatch: "Wrong person (you/he/she)",
    spelling: "Spelling / accents",
    verb_form: "Verb form (es/soy/tengo/me gusta…)",
    missing_verb: "Missing main verb",
...
function buildSuggestionForItem(prompt, answer, lang, rubric, focusTag){
  ...
  // Normaliser for anti-parrot checks
  const nLite = (s)=> norm(String(s||""));

  function pickNoRepeat(bank){
    const arr = Array.isArray(bank) ? bank.filter(Boolean) : [];
    if(!arr.length) return "";
    state.coach = state.coach || {};
    state.coach.lastModels = Array.isArray(state.coach.lastModels) ? state.coach.lastModels : [];
    const last = new Set(state.coach.lastModels.slice(-4));
    let tries = 0;
    let pick = "";
    while(tries < 12){
      pick = arr[Math.floor(Math.random()*arr.length)];
      if(!last.has(pick)) break;
      tries++;
    }
    state.coach.lastModels.push(pick);
    if(state.coach.lastModels.length > 20) state.coach.lastModels = state.coach.lastModels.slice(-20);
    return pick;
  }
  ...
  const pType = (function(){
    ...
    if(/school|instituto|colegio|escuela/.test(pLow)) return "school";
    if(/free\s+time|tiempo\s+libre|hobby|afici[oó]n|fin\s+de\s+semana/.test(pLow)) return "freetime";
    if(/yourself|describe\s+yourself|t[uú]\b\s+(?:mismo|misma)|yo\b/.test(pLow)) return "self";
    return "general";
  })();
  ...
  // One-line banks (snappy building blocks). Big enough to avoid being sussed.
  const BANK = {
    self: [
      "Soy una persona bastante trabajadora y simpática.",
      "Me considero responsable y bastante sociable.",
      "Soy tranquilo pero también divertido con mis amigos.",
      "En general soy optimista y me esfuerzo en clase.",
      "Soy bastante creativo y me gusta aprender cosas nuevas.",
      "Soy una persona amable y a veces un poco tímida."
    ],
    best_friend: [
      "Mi mejor amigo es alto y muy divertido.",
      "Mi mejor amiga es muy simpática y siempre ayuda a los demás.",
      "Se llama Marcos y es bastante responsable.",
      "Es muy trabajador y le gusta mucho el deporte.",
      "Es una persona tranquila pero con buen sentido del humor.",
      "Mi mejor amigo es amable y hablamos todos los días."
    ],
    teacher: [
      "Mi profesora es muy justa y explica las cosas con calma.",
      "Mi profe es exigente pero siempre ayuda cuando hace falta.",
      "Es amable y hace la clase más interesante.",
      "Mi profesor es paciente y nos anima a mejorar.",
      "Es estricto, pero enseña muy bien.",
      "Mi profesora es simpática y da buenos consejos."
    ],
    family: [
      "En mi familia somos cuatro y nos llevamos muy bien.",
      "Tengo un hermano y una hermana, y somos muy unidos.",
      "Vivo con mis padres y mi hermano pequeño.",
      "Mi familia es muy importante para mí.",
      "En casa solemos cenar juntos y hablamos del día.",
      "Tengo una familia grande y hacemos muchas cosas juntos."
    ],
    bedroom: [
      "Mi habitación es pequeña pero cómoda.",
      "En mi dormitorio hay una cama y un escritorio para estudiar.",
      "Mi habitación es luminosa y tengo un armario grande.",
      "Mi dormitorio es ordenado y me gusta estar allí.",
      "Mi habitación es bastante moderna y muy tranquila.",
      "En mi habitación hay pósters y una estantería."
    ],
    classroom: [
      "Mi aula es grande y bastante luminosa.",
      "En mi clase hay muchas mesas y una pizarra grande.",
      "Mi aula es cómoda y normalmente trabajamos en silencio.",
      "La clase es moderna y tenemos un proyector.",
      "Mi aula es pequeña pero acogedora.",
      "En mi clase hay ventanas grandes y mucha luz."
    ],
    town: [
      "Vivo en un pueblo pequeño y es bastante tranquilo.",
      "Mi ciudad es bonita y hay muchas tiendas en el centro.",
      "En mi barrio hay un parque donde quedo con mis amigos.",
      "Mi pueblo es seguro y la gente es muy amable.",
      "Hay un polideportivo cerca de mi casa y voy a menudo.",
      "Me gusta mi ciudad porque siempre hay cosas que hacer."
    ],
    school: [
      "Mi instituto es grande y los profesores son justos.",
      "Me gusta mi escuela porque hay buen ambiente.",
      "El colegio es moderno y tenemos buenas instalaciones.",
      "En mi instituto hay muchas actividades después de clase.",
      "Mi escuela es exigente, pero aprendo mucho.",
      "Me gusta mi instituto porque hacemos deporte a menudo."
    ],
    freetime: [
      "En mi tiempo libre juego al fútbol con mis amigos.",
      "Normalmente escucho música y veo series por la noche.",
      "A veces salgo con mis amigos, pero no todos los días.",
      "Los fines de semana me gusta descansar y hacer deporte.",
      "Me encanta leer y también uso el móvil para música.",
      "Después de clase suelo entrenar y luego hago los deberes."
    ],

    // Food / drink (likes & habits)
    food: [
      "Me gusta comer patatas porque son ricas.",
      "Me encanta la fruta porque es saludable.",
      "Me gusta desayunar cereales con leche.",
      "Normalmente como arroz y pollo en casa.",
      "A veces como pizza, pero no todos los días.",
      "Me gusta beber agua porque es mejor para la salud."
    ],

    // Favourite place at home / rooms
    house_place: [
      "Mi lugar favorito en casa es mi habitación porque es tranquila.",
      "Mi sitio favorito es el salón porque puedo relajarme allí.",
      "Me gusta la cocina porque siempre hay buen ambiente.",
      "Mi lugar favorito es mi cuarto porque tengo mis cosas.",
      "Me gusta el comedor porque cenamos juntos.",
      "Mi sitio favorito es el jardín porque hay espacio."
    ],
    kitchen: [
      "La cocina es mi lugar favorito porque siempre hablamos allí.",
      "Me gusta la cocina porque es amplia y luminosa.",
      "En la cocina hay una mesa grande y es muy práctica.",
      "La cocina es cómoda y normalmente cocinamos juntos."
    ],
    bathroom: [
      "El baño es pequeño pero limpio y cómodo.",
      "Me gusta el baño porque está siempre ordenado.",
      "El baño es moderno y tiene una ducha grande."
    ],
    living_room: [
      "El salón es cómodo y vemos la tele allí.",
      "Me gusta el salón porque es grande y tranquilo.",
      "En el salón hay un sofá grande y es muy acogedor."
    ],
    general: [
      "Es bastante bonito y me gusta mucho.",
      "En general es interesante y útil.",
      "Me gusta porque es agradable y tranquilo.",
      "Es pequeño, pero tiene todo lo necesario.",
      "Me gusta mucho y voy allí a menudo.",
      "Es cómodo y me siento bien allí."
    ]
  };

  // Choose bank by prompt type (fallback to general)
  let bank = BANK[pType] || BANK.general;

  // Focus-based steering (without parroting)
  if(focusTag==="person_mismatch"){
    // force he/she structure
    bank = (pType==="teacher") ? BANK.teacher : (pType==="best_friend") ? BANK.best_friend : (pType==="family") ? BANK.family : bank;
  } else if(focusTag==="missing_be"){
    // pick lines that clearly include es/está/son/soy
    bank = bank.filter(x=>/\b(es|está|son|soy|eres|somos)\b/i.test(x));
    if(!bank.length) bank = BANK.general;
  } else if(focusTag==="no_connector"){
    // pick connector lines
    const withConn = (BANK[pType]||BANK.general).filter(x=>/\b(y|pero|porque|además)\b/i.test(x));
    bank = withConn.length ? withConn : bank;
  }

  let model = pickNoRepeat(bank);

  // Anti-parrot: never return the learner's sentence as model
  if(aRaw){
    const an = nLite(aRaw);
    let tries = 0;
    while(tries < 8 && model && an && nLite(model)===an){
      model = pickNoRepeat(bank);
      tries++;
    }
  }
...
function jcMarking(promptText, answer, level, lang, rubric, det){
  ...
  const hard = ["verb_form","verb_ending","word_order","missing_verb","missing_be"];
...
// Choose an item-level focus tag so the exemplar matches the real leak (verbs vs detail, etc.).
const _tags = (det && Array.isArray(det.tags)) ? det.tags : [];
const _priority = ["blank","gibberish","person_mismatch","missing_verb","verb_form","verb_ending","word_order","missing_be","articles_gender","agreement","articles","spelling","no_connector","too_short","detail"];
const itemFocusTag = _priority.find(t=>_tags.includes(t)) || (_tags[0]||"detail");
const coachModel = buildSuggestionForItem(p.text, "", state.lang, rubric, itemFocusTag);
...
// Build per-item quick fixes (snappy hints, not copy-paste answers)
state.mark.items = state.mark.items.map(it=>{
  if(it.ok) return {...it, fix:"", itemFocusTag:"", why: it.why || ""};

  const tags = Array.isArray(it.tags)?it.tags:[];
  const priority = ["blank","gibberish","person_mismatch","verb_form","verb_ending","word_order","missing_verb","missing_be","articles_gender","agreement","articles","spelling","no_connector","too_short","detail"];
  const focusTag = priority.find(t=>tags.includes(t)) || (tags[0]||"detail");

  // --- Fix hints (never a full answer) ---
  const bullets = [];
  const ansLow = String(it.answer||"").toLowerCase();

  if(state.lang==="es"){
    if(focusTag==="blank"){
      bullets.push("Write ONE clean sentence (8+ words).");
    } else if(focusTag==="gibberish"){
      bullets.push("Turn notes into ONE sentence with a verb (es/tengo/hay/me gusta…).");
      bullets.push("No lists. No numbers. One line.");
    } else if(focusTag==="person_mismatch"){
      bullets.push("Wrong person: describe him/her (él/ella es…), not ‘you are’.");
      bullets.push("Starter: “Mi mejor amigo(a) es…” / “Mi profesor(a) es…”.");
    } else if(focusTag==="missing_verb"){
      bullets.push("Add a main verb (not a list).");
      bullets.push("Patterns: “Me gusta(n) ____.” / “Hay ____.” / “Tengo ____.”");
    } else if(focusTag==="missing_be"){
      bullets.push("Add a ‘to be’ verb: es/está/son/soy…");
      bullets.push("Pattern: “Mi ____ es ____.”");
    } else if(focusTag==="verb_form"){
      if(/\beres\b/.test(ansLow)) bullets.push("Swap eres → es (he/she) or soy (I), depending on the prompt.");
      bullets.push("Use a real verb form (es/tiene/le gusta…).");
    } else if(focusTag==="verb_ending"){
      bullets.push("Conjugate the verb: yo hablo / él habla (not hablar).");
    } else if(focusTag==="word_order"){
      bullets.push("Word order: me gusta + infinitive (me gusta jugar).");
    } else if(focusTag==="agreement"){
      bullets.push("Match endings: habitación/clase → cómoda/bonita (not cómodo/bonito).");
    } else if(focusTag==="articles_gender"){
      bullets.push("Use el/la, un/una and match the noun.");
    } else if(focusTag==="spelling"){
      const fx = wordFixesES(it.answer);
      if(fx.changes && fx.changes.length){
        const words = fx.changes.slice(0,3).map(x=>x.split("→")[1].trim()).join(", ");
        bullets.push("Fix these words: " + words);
      } else {
        bullets.push("Fix spelling/accents on key words.");
      }
    } else if(focusTag==="no_connector"){
      bullets.push("Add ONE connector: y / pero / porque / además.");
    } else { // detail / too_short
      bullets.push("Add ONE extra detail (where / frequency / reason).");
      bullets.push("Keep it one clean sentence.");
    }
  } else {
    bullets.push("Make it clean. Add one extra detail.");
  }

  const fix = bullets.filter(Boolean).slice(0,2).map(x=>"• "+x).join("  |  ");

  // --- Why (short + specific) ---
  let why = it.why || "";
  if(focusTag==="person_mismatch") why = "Wrong person costs credit: you’re describing someone else.";
  else if(focusTag==="gibberish") why = "Notes don’t score. One clean sentence does.";
  else if(focusTag==="verb_form") why = "Verb form is the biggest mark-leak here.";
  else if(focusTag==="verb_ending") why = "Unconjugated verbs cost marks fast.";
  else if(focusTag==="word_order") why = "Spanish word order matters (me gusta…).";
  else if(focusTag==="missing_be") why = "Descriptions need es/está/son/soy…";
  else if(focusTag==="agreement") why = "Agreement endings must match (alto/alta).";
  else if(focusTag==="articles_gender") why = "Articles must match gender (el/la, un/una).";
  else if(focusTag==="spelling" && it.examples && it.examples.length) why = it.examples.slice(0,3).join(" • ");
  else if(focusTag==="no_connector") why = "Linking ideas raises credit immediately.";
  else if(focusTag==="too_short" || focusTag==="detail") why = "One extra controlled detail lifts the score.";
...
function gymFocusType(){
  ...
  if(tag==="person_mismatch") return "verbs";
  if(tag==="gibberish") return "detail";
  if(tag==="blank") return "detail";
  ...
  if(tag==="missing_verb") return "verbs";
...
function isBadGymSeed(ans){
  ...
  if(ratio < 0.55 || digits >= 2 || weird >= 2) return true;

  // Too many tiny fragments like "es 4. yo, hhermano..."
  const toks = t.split(/\s+/).filter(Boolean);
  const short = toks.filter(x=>x.length<=2).length;
  if(toks.length>=5 && short/toks.length > 0.55) return true;
...
state.workshop.recentRepSigs = [];
  ...
  state.workshop.justOpened = true;
  ...
  function nextGymDrill(){
    // Pick a fresh source item to avoid repetition and keep drills tied to the learner's own work
        // First rep: stay tied to the exact sentence/prompt that triggered the Gym (reduces mismatch + keeps trust)
    if(state.workshop && state.workshop.justOpened && state.workshop.refItem){
      state.workshop.currentItem = state.workshop.refItem;
      state.workshop.justOpened = false;
    }else{
...
if(type==="detail" || type==="upgrade"){
        ...
        const baseModel = pickModelAnswer({text: ptxt || "Describe something."});
        const base = tidySuggestion(String(baseModel||model||weak).split(".")[0] + ".");
...
"${basePrompt||"die Aufgabe"}"`;
      ...
      if(el.wsHelp){
        ...
        // Strong hints without giving the exact sentence
        const starter = (function(){
          const p = String(basePrompt||"").toLowerCase();
          if(/best\s+friend|amig[oa]\s+(?:mejor|favorit)/.test(p)) return ["Mi mejor amigo es ___", "Se llama ___ y es ___", "En mi opinión, es ___ porque ___"];
          if(/teacher|profesor|profe\b/.test(p)) return ["Mi profesor(a) es ___", "Me gusta mi profe porque ___", "En clase es ___ y ___"];
          if(/family|familia/.test(p)) return ["En mi familia somos ___", "Tengo ___ y ___", "Nos llevamos ___ porque ___"];
          if(/bedroom|habitaci[oó]n|dormitorio/.test(p)) return ["Mi habitación es ___", "En mi habitación hay ___", "Tengo ___ para ___"];
          if(/classroom|aula|clase/.test(p)) return ["Mi aula es ___", "En mi clase hay ___", "La clase es ___ y ___"];
          if(/town|city|pueblo|ciudad|barrio/.test(p)) return ["Vivo en ___ y es ___", "En mi barrio hay ___", "Me gusta porque ___"];
          if(/school|instituto|colegio|escuela/.test(p)) return ["Mi instituto es ___", "Me gusta mi escuela porque ___", "Hay ___ y ___"];
          return ["Start with: “Mi ____ es ____”", "Add: porque ___", "Add: y además ___"];
        })();

        lines.push("Starters: " + starter.slice(0,2).join(" / "));
...
if(type==="connector"){
      ...
      const wordsOk = countWords(val) >= Math.max(6, Math.min(10, lvlRub.minWords));
      const coherent = !isBadGymSeed(val);
      const hasVerb = beVerbPresent(val) || /\b(tengo|hay|me\s+gusta|quiero|puedo|voy|suelo|hago|juego|vivo)\b/i.test(val);
      ok = wordsOk && coherent && hasVerb;
      msgOk = "Nice — stronger line.";
      msgNo = "Try again: one clean sentence with a verb + one extra detail (6+ words).";
    } else {
