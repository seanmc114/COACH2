...
const COACH = {
  // Calm, believable, slightly sarcastic — but always on the learner’s side.
  ...
  // Quick “touchline” lines
  praise: [
    "Bien. That’s a proper line.",
    "Good. Clean and clear — keep it.",
    "Solid. No chaos, no guessing.",
    "That’s control. I respect control.",
    "You’re improving. Don’t get lazy.",
    "Much better. I can actually read it.",
    "Yes. That would score.",
    "Good habits. Repeat them.",
    "Nice. You didn’t panic — you built it.",
    "Alright. You can play."
  ],
  push: [
    "Standards. We raise them every round.",
    "Details win matches. You want wins, yes?",
    "Same idea — cleaner language.",
    "One weakness. We fix it. Then we move.",
    "Close is not the score.",
    "You’re nearly there… don’t throw it away.",
    "Calm. One fix. Then points.",
    "Stop gambling with endings.",
  ...
  jail: [
    ...
    "No excuses. Reps. Then back out.",
    "Quick session. Earn the exit.",
    "Tunnel time. Fix the leak.",
    "We don’t argue with the marks — we train.",
    "Reset. One focus, five clean tries.",
    "You want freedom? Show me control.",
    "Back in. One problem. Fix it.",
    "Right — boots on. Gym."
  ],
  release: [
    "Good. Exit earned.",
    ...
    "You’re back out. Same discipline now.",
    "Nice. The fix worked.",
    "Good. Now we build on it.",
    "Alright — back to the match.",
    "Clean. That’s how you score.",
    "Okay. You can leave the gym."
  ],

  // Extra variety for the results coach modal
  verdictWin: [
    "Good. Now polish it — we don’t celebrate mess.",
    "You won the round. Now win the details.",
    "Better. Keep it tidy and you’ll fly.",
    "That’s a score. Now make it repeatable.",
    "Nice. But I want it cleaner, not luckier.",
    "Good work. Don’t switch off.",
    "Yes. That’s how you get points.",
    "Solid. Now do it again under pressure.",
    "Alright. You can breathe — then fix one thing.",
    "Good. Professional, not perfect — yet."
  ],
  verdictLose: [
    "Not enough. Reset. One focus.",
    "No excuses. Fix the cog and go again.",
    "We don’t drift. We correct.",
    "That costs points. We remove it.",
    "Calm. This is fixable — one change.",
    "Not today. But we learn fast.",
    "Too messy. Make it simple and correct.",
    "You gave me vibes, not marks.",
    "Workmanlike, not wild — try again.",
    "You can do this. Stop rushing."
  ],
  line2MustGym: [
    "Gym is required. Earn the exit — then you come back sharper.",
    "Gym is locked in. Five reps, then we continue.",
    "Straight to gym. Fix it once and it stays fixed.",
    "Gym time. Quick and focused. Then back out.",
    "No shortcuts today — gym first, then match."
  ],
  line2OptionalGym: [
    "Gym is optional. Review feedback, then choose reps.",
    "You can take gym or take results — your call.",
    "Up to you: read the fixes, or drill them.",
    "You’re close. Gym helps, but it’s your decision.",
    "Look at the model — then decide if you want practice."
  ],

  // Short “matchday” warnings per level (used in coach messages).
  levelWarnings: [
    "Level 1: one clean sentence + one detail.",
    "Level 2: use the right ‘to be’ — don’t dodge it.",
    "Level 3: one connector, but only if it’s correct.",
    "Level 4: because/but/and — choose properly.",
    "Level 5: endings matter — gender/number.",
    "Level 6: opinions need a reason.",
    "Level 7: compare neatly (más/menos/igual).",
    "Level 8: plans — don’t guess the future.",
    "Level 9: make it flow (primero / luego / después).",
    "Boss: variety + accuracy. No panic."
  ]
};
...
// Coach variety: avoid repeating the exact same line over and over.
const LS_PICK_HISTORY = "loops_pickHistory_v1";
function pickVar(key, arr, keep=2){
  try{
    arr = Array.isArray(arr) ? arr : [];
    if(!arr.length) return "";
    let hist = {};
    try{ hist = JSON.parse(storage.get(LS_PICK_HISTORY) || "{}") || {}; }catch(_){ hist = {}; }
    const prev = Array.isArray(hist[key]) ? hist[key] : [];
    let idx = 0;

    // If the bank is tiny, just pick normally.
    if(arr.length <= keep + 1){
      idx = Math.floor(Math.random()*arr.length);
    }else{
      // Try a few times to avoid recent repeats.
      for(let tries=0; tries<14; tries++){
        const cand = Math.floor(Math.random()*arr.length);
        if(prev.indexOf(cand) === -1){ idx = cand; break; }
        idx = cand;
      }
    }

    hist[key] = [idx].concat(prev).slice(0, keep);
    try{ storage.set(LS_PICK_HISTORY, JSON.stringify(hist)); }catch(_){}
    return arr[idx];
  }catch(_){
    return pick(arr);
  }
...
function presentCoachModal(){
  try{
    ...
    const mustGym = !!state.gymRequired;
        ...
        const vibe = passed
      ? pickVar("verdict_win", COACH.verdictWin || COACH.praise)
      : pickVar("verdict_lose", COACH.verdictLose || COACH.push);

    const line2 = mustGym
      ? pickVar("line2_must", COACH.line2MustGym || COACH.jail)
      : pickVar("line2_opt", COACH.line2OptionalGym || COACH.push);

    // Why + drill are intentionally narrow: one main error, then reps.
    const reasonMap = {
      spelling: [
        "Spelling/accents are free points. Clean them and everything looks smarter.",
        "Accents matter. One missing accent can turn a good idea into a lost mark.",
        "This is not a big grammar problem — it’s a polish problem. Fix the polish."
      ],
      verb_form: [
        "Wrong verb form breaks the sentence. Fix the engine first.",
        "Your meaning is fine — your verb choice is not. Fix the verb.",
        "You have the idea. Put it into a correct verb frame."
      ],
      verb_ending: [
        "Endings matter: yo como / él come. Choose the right ending.",
        "Your sentence is almost right — the ending is leaking points.",
        "Pick the person and match the ending. Simple."
      ],
      agreement: [
        "Agreement: adjective endings must match (alto/alta).",
        "Your adjective is fighting your noun. Make them agree.",
        "Match it: feminine/masculine, singular/plural — then it scores."
      ],
      articles_gender: [
        "Articles + gender are small, but they cost marks when wrong. Tighten them up.",
        "Get el/la and un/una right — it instantly sounds more accurate.",
        "This is a quick win: article + noun must match."
      ],
      articles: [
        "Articles make Spanish sound natural (un/una, el/la).",
        "Missing/incorrect articles make it feel incomplete. Add the right one.",
        "One article in the right place = instant improvement."
      ],
      word_order: [
        "Word order changes meaning. Use the pattern, then it’s automatic.",
        "You’re thinking in English order. Swap to the Spanish pattern.",
        "Get the structure right first — then add style."
      ],
      missing_be: [
        "Descriptions need the ‘to be’ verb (ser/estar). Put it in.",
        "You’re describing… but you forgot the ‘is/are’. Fix that.",
        "Add the ‘to be’ verb and the whole line becomes valid."
      ],
      no_connector: [
        "One connector makes your ideas flow (y/pero/porque/además).",
        "Link ideas neatly — one correct connector is enough.",
        "You don’t need a long sentence. Just connect two ideas correctly."
      ],
      too_short: [
        "Too short = lost marks. Add one detail and you jump up.",
        "One extra fact turns a fragment into a proper answer.",
        "Give me a full line: main idea + one detail."
      ],
      detail: [
        "Detail wins marks. Add one extra fact.",
        "Good start — now add one detail that proves you know the topic.",
        "One more piece of information. That’s the difference."
      ]
    };

    const drillMap = {
      spelling: [
        "Drill: clean spelling/accents.",
        "Drill: fix accents (á/é/í/ó/ú) — quick reps.",
        "Drill: polish the line (same sentence, correct accents)."
      ],
      verb_form: [
        "Drill: key verbs (soy/tengo/me gusta).",
        "Drill: pick the right verb frame (ser/estar/tener/gustar).",
        "Drill: rebuild the sentence with the correct verb."
      ],
      verb_ending: [
        "Drill: conjugate endings (yo / él / nosotros).",
        "Drill: match subject + ending.",
        "Drill: change person without changing meaning."
      ],
      agreement: [
        "Drill: adjective agreement (alto/alta, simpático/simpática).",
        "Drill: make noun + adjective agree.",
        "Drill: swap gender/plural correctly."
      ],
      articles_gender: [
        "Drill: articles + gender (el/la, un/una).",
        "Drill: article + noun matching.",
        "Drill: choose el/la quickly, then write the noun."
      ],
      articles: [
        "Drill: articles (un/una, el/la).",
        "Drill: add the missing article.",
        "Drill: pick the correct article in context."
      ],
      word_order: [
        "Drill: word order patterns (me gusta…, en mi ciudad…).",
        "Drill: rebuild with Spanish structure.",
        "Drill: move the pieces into the right order."
      ],
      missing_be: [
        "Drill: ser/estar (is/are).",
        "Drill: add ‘to be’ correctly.",
        "Drill: choose ser vs estar, then write the line."
      ],
      no_connector: [
        "Drill: connectors (y/pero/porque/además).",
        "Drill: link two ideas with one connector.",
        "Drill: choose the right connector for meaning."
      ],
      too_short: [
        "Drill: add one extra detail.",
        "Drill: main idea + one detail.",
        "Drill: extend the line by one useful fact."
      ],
      detail: [
        "Drill: add one extra detail.",
        "Drill: add place/time/reason (one only).",
        "Drill: upgrade a basic line with one detail."
      ]
    };

    const focusWhy = Array.isArray(reasonMap[focusTag]) ? pickVar(`why_${focusTag}`, reasonMap[focusTag]) : (reasonMap[focusTag] || "One thing. Fix it properly.");
    const drillLine = Array.isArray(drillMap[focusTag]) ? pickVar(`drill_${focusTag}`, drillMap[focusTag]) : (drillMap[focusTag] || "Quick reps. Then back in.");
...
function detectTags(prompt, answer, lang, rubric){
  ...
  if(lang==="es"){
    ...
    // Common mismatch: subject + wrong form of "ser/estar" (e.g., él eres / yo es)
    if(/\b(el|él)\s+eres\b/.test(low)){ tags.push("verb_form"); tags.push("verb_ending"); examples.push("él eres → él es"); }
    if(/\b(tu|tú)\s+es\b/.test(low)){ tags.push("verb_form"); tags.push("verb_ending"); examples.push("tú es → tú eres"); }
    if(/\b(yo)\s+eres\b/.test(low)){ tags.push("verb_form"); tags.push("verb_ending"); examples.push("yo eres → yo soy"); }

    // "me gusta" should be followed by an infinitive or a noun, not a conjugated verb
    if(/\bme\s+gusta\s+(tengo|tienes|tiene|tenemos|ten[eé]is|tienen|soy|eres|es|somos|son|estoy|est[aá]s|est[aá]|estamos|est[aá]n|voy|vas|va|vamos|van)\b/.test(low)){
      tags.push("verb_form");
      tags.push("word_order");
      examples.push("me gusta + infinitivo/sustantivo (no + verbo conjugado)");
    }
...
function tipForTags(tags, lang){
  ...
  if(lang==="es"){
    ...
    if(has("verb_form")) return pickVar("tip_verb_form", ["Fix the verb frame: soy / tengo / me gusta.", "Start with the engine: soy / tengo / me gusta — then add detail.", "Get the verb right first. Everything else follows."]);
    if(has("verb_ending")) return pickVar("tip_verb_ending", ["Conjugate it: yo como / él come.", "Match the subject + ending (yo / él / nosotros).", "Endings are points — don’t throw them away."]);
    if(has("word_order")) return pickVar("tip_word_order", ["Word order: me gusta + infinitivo.", "Use the pattern: me gusta + infinitivo.", "Spanish structure: me gusta…, en mi ciudad…"]);
    if(has("articles_gender")) return pickVar("tip_articles_gender", ["Article + gender: el/la, un/una — match the noun.", "Quick win: choose el/la correctly.", "Article + noun must match. Easy points."]);
    if(has("agreement")) return pickVar("tip_agreement", ["Agreement: alto/alta, simpático/simpática.", "Make the adjective match the noun.", "Swap gender/plural correctly."]);
    if(has("missing_be")) return pickVar("tip_missing_be", ["Add ser/estar: es / está / soy…", "Don’t forget the ‘is/are’ — es/está.", "Put the ‘to be’ verb in. Then it works."]);
    if(has("no_connector")) return pickVar("tip_no_connector", ["Add one connector: y / pero / porque / además.", "Link two ideas with one correct connector.", "One connector is enough — just make it correct."]);
    if(has("spelling")) return pickVar("tip_spelling", ["Accents matter: á é í ó ú (and ñ).", "Polish: fix accents and spelling.", "Clean accents = free marks."]);
    if(has("too_short")) return pickVar("tip_too_short", ["Add one extra detail (place/activity/opinion).", "Main idea + one detail. That’s it.", "Give one extra fact. Instant upgrade."]);
    return pickVar("tip_default_es", ["Add one detail. Keep it correct.", "Clean line first, detail second.", "One good sentence beats three messy ones."]);
  }
...
function buildSuggestionForItem(prompt, answer, lang, rubric, focusTag){
  ...
  const pRaw = String(prompt||"").trim();
    ...
    // Detect topic from prompt text (English/Spanish mixed).
    const is = (re)=> re.test(s);
    const cat =
      is(/telephone|tel[ée]fono|teléfono|movil|móvil|mobile|cell(phone)?|smartphone/) ? "phone" :
      is(/town|pueblo|ciudad|barrio|zona|donde vives|dónde vives/) ? "town" :
      is(/school|colegio|instituto|escuela|mi colegio|mi instituto/) ? "school" :
      is(/classroom|clase|aula/) ? "classroom" :
      is(/friend|amig[oa]|mejor amigo|mejor amiga/) ? "friend" :
      is(/family|familia|padre|madre|herman[oa]/) ? "family" :
      is(/bedroom|habitaci|dormitorio/) ? "bedroom" :
      is(/person.*admire|admire|admiro|modelo|h[eé]roe|héroe/) ? "admire" :
      is(/house|home|casa|hogar/) ? "house" :
      "generic";

    const POOL = {
      phone: [
        "Mi móvil es moderno y lo uso todos los días.",
        "Mi teléfono es pequeño pero muy rápido.",
        "Mi móvil es nuevo y tiene una buena cámara.",
        "Mi teléfono es práctico y me ayuda mucho.",
        "Mi móvil es barato pero funciona muy bien.",
        "Mi teléfono es negro y me encanta.",
        "Mi móvil es útil y lo uso para escuchar música.",
        "Mi teléfono es importante para mí porque hablo con mis amigos.",
        "Mi móvil es sencillo pero cómodo.",
        "Mi teléfono es genial y lo cuido mucho.",
        "Mi móvil es rápido y tiene muchas apps.",
        "Mi teléfono es viejo pero todavía va bien."
      ],
      town: [
        "Vivo en un pueblo pequeño y es bastante tranquilo.",
        "Vivo en una ciudad grande y siempre hay algo que hacer.",
        "Mi barrio es tranquilo y la gente es amable.",
        "Mi pueblo es bonito y está cerca del centro.",
        "Mi ciudad es moderna y tiene muchas tiendas.",
        "Vivo en una zona residencial y es muy segura.",
        "Mi pueblo es antiguo y tiene un castillo.",
        "Mi ciudad es animada y me gusta mucho.",
        "Mi barrio es ruidoso pero divertido.",
        "Vivo cerca del centro y es muy cómodo.",
        "Mi ciudad es verde y hay muchos parques.",
        "Mi pueblo está cerca del mar y es precioso."
      ],
      school: [
        "Mi colegio es grande y me gusta bastante.",
        "Mi instituto es moderno y tiene buenas instalaciones.",
        "Mi colegio es antiguo pero es muy bueno.",
        "Mi escuela es pequeña y los profesores son simpáticos.",
        "Mi instituto es grande y hay muchas actividades.",
        "Mi colegio es exigente pero aprendo mucho.",
        "Mi escuela es tranquila y se trabaja bien.",
        "Mi instituto está cerca de mi casa y es cómodo.",
        "Mi colegio es famoso y estoy orgulloso.",
        "Mi escuela es divertida y tengo buenos amigos allí.",
        "Mi instituto es bonito y hay un patio grande.",
        "Mi colegio es serio, pero el ambiente es bueno."
      ],
      classroom: [
        "Mi clase es luminosa y es bastante cómoda.",
        "El aula es pequeña pero está bien organizada.",
        "Mi clase es grande y hay muchas mesas.",
        "El aula es moderna y tiene una pizarra digital.",
        "Mi clase es tranquila y se trabaja bien.",
        "El aula es un poco ruidosa, pero es divertida.",
        "Mi clase es bonita y hay pósters en la pared.",
        "El aula es cómoda y me gusta aprender allí.",
        "Mi clase es fría en invierno, pero está bien.",
        "El aula es limpia y está ordenada.",
        "Mi clase es agradable y hay buen ambiente.",
        "El aula es espaciosa y tenemos sitio."
      ],
      friend: [
        "Mi mejor amigo es simpático y es muy divertido.",
        "Mi mejor amiga es amable y siempre me ayuda.",
        "Mi amigo es trabajador y es muy inteligente.",
        "Mi amiga es deportista y le encanta el fútbol.",
        "Mi mejor amigo es alto y tiene buen sentido del humor.",
        "Mi mejor amiga es baja y es muy habladora.",
        "Mi amigo es creativo y es muy buena persona.",
        "Mi amiga es tranquila pero muy graciosa.",
        "Mi mejor amigo es sociable y tiene muchos amigos.",
        "Mi mejor amiga es leal y confío en ella.",
        "Mi amigo es serio en clase pero divertido fuera.",
        "Mi amiga es simpática y me cae genial."
      ],
      family: [
        "En mi familia somos cuatro y nos llevamos bien.",
        "Tengo un hermano y una hermana y son muy diferentes.",
        "Vivo con mis padres y somos una familia unida.",
        "Mi familia es grande y somos muy cercanos.",
        "En mi casa vivimos cinco y hay buen ambiente.",
        "Mi familia es divertida y hacemos cosas juntos.",
        "Tengo dos hermanos y a veces discutimos, pero nos queremos.",
        "Mi familia es tranquila y nos ayudamos mucho.",
        "Vivo con mi madre y mi padre y son muy trabajadores.",
        "Mi familia es muy importante para mí.",
        "En mi familia hay mucho ruido, pero me gusta.",
        "Somos pocos en casa, pero somos muy unidos."
      ],
      bedroom: [
        "Mi habitación es pequeña pero es cómoda.",
        "Mi dormitorio es grande y está bastante ordenado.",
        "Mi habitación es luminosa y me gusta mucho.",
        "Mi dormitorio es sencillo, pero tengo lo necesario.",
        "Mi habitación es moderna y hay una cama cómoda.",
        "Mi dormitorio es tranquilo y puedo estudiar allí.",
        "Mi habitación es un poco desordenada, pero es mi sitio.",
        "Mi dormitorio tiene pósters y es muy personal.",
        "Mi habitación es acogedora y me relaja.",
        "Mi dormitorio es frío en invierno, pero está bien.",
        "Mi habitación tiene un escritorio y lo uso mucho.",
        "Mi dormitorio es bonito y tiene buena luz."
      ],
      admire: [
        "Admiro a mi madre porque es muy trabajadora.",
        "Admiro a mi padre porque siempre me apoya.",
        "Admiro a mi abuela porque es muy fuerte.",
        "Admiro a mi hermano porque es muy constante.",
        "Admiro a un profesor porque explica muy bien.",
        "Admiro a un deportista porque entrena mucho.",
        "Admiro a una persona famosa porque es solidaria.",
        "Admiro a mi mejor amigo porque es leal.",
        "Admiro a mi hermana porque es muy inteligente.",
        "Admiro a mi familia porque siempre está conmigo.",
        "Admiro a mi entrenador porque me motiva.",
        "Admiro a mi madre porque nunca se rinde."
      ],
      house: [
        "Mi casa es bastante grande y es muy cómoda.",
        "Vivo en una casa pequeña, pero es acogedora.",
        "Mi casa es moderna y tiene un salón amplio.",
        "Mi casa es antigua, pero me encanta.",
        "Vivo en un piso y está cerca del centro.",
        "Mi casa tiene un jardín y me gusta mucho.",
        "Mi casa es luminosa y hay buen ambiente.",
        "Vivo en un piso pequeño, pero es práctico.",
        "Mi casa es tranquila y puedo descansar.",
        "Mi casa está en las afueras y es silenciosa.",
        "Mi casa es bonita y está bien situada.",
        "Vivo en un piso moderno y es muy cómodo."
      ],
      generic: [
        "Soy una persona simpática y me gusta aprender.",
        "Me gusta mi vida porque tengo buenos amigos.",
        "En general estoy contento y soy bastante tranquilo.",
        "Soy trabajador y quiero sacar buenas notas.",
        "Me considero amable y soy bastante sociable.",
        "Soy un poco tímido, pero soy buena persona.",
        "Me gusta el deporte y soy bastante activo.",
        "Soy creativo y me gusta la música.",
        "Soy serio cuando toca, pero divertido también.",
        "Me gusta salir con mis amigos los fines de semana.",
        "Soy responsable y quiero mejorar mi español.",
        "Soy una persona normal, pero con ganas de mejorar."
      ]
    };

    return pickVar(`model_${cat}`, POOL[cat] || POOL.generic);
  }
  ...
  function pickDetailAddonES(p){
    ...
    const cat =
      is(/telephone|tel[ée]fono|teléfono|movil|móvil|mobile|cell(phone)?|smartphone/) ? "phone" :
      is(/town|pueblo|ciudad|barrio|zona|donde vives|dónde vives/) ? "town" :
      is(/school|colegio|instituto|escuela|classroom|clase|aula/) ? "school" :
      is(/friend|amig[oa]|mejor amigo|mejor amiga/) ? "friend" :
      is(/family|familia|padre|madre|herman[oa]/) ? "family" :
      is(/bedroom|habitaci|dormitorio/) ? "bedroom" :
      is(/house|home|casa|hogar/) ? "house" :
    ...
    const ADD = {
      phone: [
        ...
        "tengo muchas aplicaciones",
        "lo uso para mandar mensajes",
        "lo uso para escuchar música",
        "lo cargo cada noche",
        "me sirve para estudiar a veces",
        "es rápido y práctico",
        "me encanta hacer fotos",
        "lo uso para hablar con mis amigos",
        "tiene mucha memoria",
        "es sencillo pero útil",
        "me ayuda todos los días"
      ],
      town: [
        "hay un parque cerca",
        "hay un centro comercial",
        "hay buenas cafeterías",
      ...
      friend: [
        ...
        "es muy sociable",
        "tiene el pelo corto",
        "me ayuda con los deberes",
        "jugamos juntos al fútbol",
        "es muy trabajador",
        "es muy simpático",
        "confío mucho en él/ella"
      ],
      family: [
        "nos llevamos muy bien",
        "cenamos juntos a menudo",
        "nos ayudamos en casa",
        "a veces discutimos, pero es normal",
        "hacemos cosas juntos los fines de semana",
        "mi familia es muy unida",
        "me apoyan mucho",
        "somos muy cercanos",
        "hay buen ambiente en casa",
        "mi familia es divertida",
        "somos bastante tranquilos",
    ...
    return pickVar(`detail_${cat}`, ADD[cat] || ADD.generic);
  }
  ...
  if(lang==="es"){
    ...
    if(focusTag==="articles_gender" || focusTag==="articles" || focusTag==="agreement"){
      ...
      return wordFixesES(x).fixed;
        ...
        // Detail focus: add ONE extra detail, contextual to prompt (keep it as ONE line).
    if(focusTag==="too_short" || focusTag==="detail"){
      ...
      const seed = (baseWc < Math.max(4, (rubric?.minWords||6)-2))
        ? modelFromPromptES(pRaw, Number(state.level)||1).replace(/[.!?]+$/,".")
        : base;

      const seedNoP = seed.replace(/[.!?]+$/,"");
      const joiners = ["y", "además", "también"];
      const j = pickVar("joiner", joiners);
      const adClean = ad ? (ad[0].toLowerCase() + ad.slice(1)) : "me gusta mucho";

      if(j==="además") return `${seedNoP}; además, ${adClean}.`;
      if(j==="también") return `${seedNoP} y también ${adClean}.`;
      return `${seedNoP} y ${adClean}.`;
    }

    // Connector focus: add a connector appropriately (keep it as ONE line).
    if(focusTag==="no_connector"){
      const base = wordFixesES(a).fixed.replace(/[.!?]+$/,"");
      const conns = ["porque","pero","y","además","entonces"];
      const c = pickVar("connector", conns);

      const tails = {
        porque: ["me gusta mucho", "es interesante", "me ayuda a relajarme", "quiero mejorar", "aprendo mucho"],
        pero: ["a veces es difícil", "no siempre tengo tiempo", "a veces es aburrido", "es un poco caro", "no está cerca"],
        y: ["me gusta mucho", "es bastante divertido", "es importante para mí", "lo hago con mis amigos", "es muy útil"],
        además: ["me gusta mucho", "es bastante útil", "hay muchas actividades", "tengo buenos amigos", "aprendo bastante"],
        entonces: ["estoy contento", "me siento bien", "mejoro cada día", "lo hago a menudo", "tengo más confianza"]
      };

      const tail = pickVar(`tail_${c}`, tails[c] || tails.y);
      const connectorText = (c==="además") ? "además" : c;
      return `${base} ${connectorText} ${tail}.`;
    }
...
const hasPorqueES  = (s)=> /\bporque\b|\bya\s+que\b/i.test(String(s||""));
...
const hasConnectorES = (s)=> /\b(y|pero|porque|además|entonces|también|sin\s+embargo|ya\s+que)\b/i.test(String(s||""));
  ...
  function unlockTargetForLevel(level){ return Math.max(90, 160 - (Number(level)||1) * 5); }
...
function jcMarking(promptText, answer, level, lang, rubric, det){
  ...
  if(wantsClean) need("Clean sentence", wc >= Math.max(4, rubric.minWords-2));
  ...
  need("Length", wc >= rubric.minWords);

  const earned = checks.filter(c=>c.ok).map(c=>c.label);
  const missed = checks.filter(c=>!c.ok).map(c=>c.label);

  // Score: start with coverage points from checks, then subtract for accuracy cogs
  let score = 2; // attempt credit
  const maxCheckPts = 8;
  const checkPts = Math.min(maxCheckPts, earned.length);
  score += checkPts; // 2..10

  // Hard language cogs reduce score sharply (JC examiner feel)
  const tags = (det && det.tags) ? det.tags : [];
  const hard = ["verb_form","verb_ending","word_order","missing_be"];
  const medium = ["articles_gender","agreement","articles"];
  if(tags.includes("blank")) score = 0;
  hard.forEach(t=>{ if(tags.includes(t)) score -= 2; });
  medium.forEach(t=>{ if(tags.includes(t)) score -= 1; });
  if(tags.includes("spelling")) score -= 1;
  if(tags.includes("no_connector") && wantsConn) score -= 1;
  ...
  else if(missed.length) note="Coverage";
...
return {...it, aiFix: betterAiFix, tip: betterTip, why: betterWhy};
    ...
    for(let i=0;i<PROMPTS_PER_ROUND;i++){ 
      ...
      let ok=true;
      let reason="";
      if(!ans || ans==="—"){ ok=false; reason="Blank"; }
      else {
...
const coachModel = buildSuggestionForItem(p.text, "", state.lang, rubric, "detail");
...
items.push({ n:i+1, prompt:p.text, answer:ans||"—", ok, reason, suggestion: coachModel, tip: t2, why, tags: det.tags, examples: det.examples,
             aiFix, score10: jm.score10, checks: jm.checks, missed: jm.missed, earned: jm.earned, note: jm.note });
    }
...
state.mark.items = state.mark.items.map(it=>{
  ...
  const vibe = m.passed ? pickVar("praise", COACH.praise) : pickVar("push", COACH.push);
  const nudge = m.passed ? pickVar("nudge", COACH.push) : "";
  const jail = state.gymRequired ? pickVar("jail", COACH.jail) : "";
  ...
  const reasonText = Array.isArray(reasonMap[focusTag]) ? pickVar(`why_${focusTag}`, reasonMap[focusTag]) : (reasonMap[focusTag] || "We fix one thing, properly.");
  const drillText  = Array.isArray(drillMap[focusTag]) ? pickVar(`drill_${focusTag}`, drillMap[focusTag]) : (drillMap[focusTag] || "Drill: quick reps.");
...
function renderResults(){
    ...
    if(el.pointsOut) el.pointsOut.textContent = String((m.items||[]).reduce((a,it)=>a+(Number(it.score10)||0),0));
    ...
    if(el.feedbackList){
      ...
      items.forEach(it=>{
        ...
        card.innerHTML = `
          ...
          <div class="fbMeta">
            <div class="fbScore">${(typeof it.score10==="number")?(`${it.score10}/10`):""}</div>
