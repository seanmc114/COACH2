...
function presentCoachModal(){
  try{
    ...
    const also = top.filter(t=>t && t.label && t.tag && t.tag!==focusTag && t.tag!=="ok" && t.tag!=="detail").slice(0,3).map(t=>t.label);

    const passed = !!m.passed;
    const mustGym = !!state.gymRequired;
    const totalPts = items.reduce((a,it)=>a + (Number(it.score10)||0), 0);
    const pct = Math.round((totalPts/(PROMPTS_PER_ROUND*10))*100);
    const timeStr = fmtTime(state.elapsedMs);
    ...
    const reasonMap = {
    no_answer: "No answer = no credit. Put ONE clean sentence down.",
    blank: "No answer = no credit. Put ONE clean sentence down.",
    starter: "You’ve started, but it’s not a sentence yet. Give it a subject + verb.",
      ...
      too_short: "A bit short — add one controlled detail to lift the mark.",
      detail: "Detail wins marks. One extra fact is the difference."
    };
    const drillMap = {
    no_answer: "Drill: one clean starter line.",
    blank: "Drill: one clean starter line.",
    starter: "Drill: turn fragments into ONE sentence.",
      ...
      too_short: "Drill: add one controlled detail.",
    ...
    const html = `
      ...
      <div class="muted" style="margin-top:4px"><b>Round score:</b> ${totalPts}/${PROMPTS_PER_ROUND*10} (${pct}%) • <b>Time:</b> ${timeStr} <span style="opacity:.85">(tie-break)</span></div>
      <div style="margin-top:10px"><b>Today’s focus:</b> ${escapeHtml(focusLabel)}</div>
...
function detectTags(prompt, answer, lang, rubric){
  ...
  if(aTrim && wc < 3) tags.push("starter");
  else if(aTrim && rubric && wc < Math.max(4, Number(rubric.minWords||4)-1)) tags.push("detail");
...
function focusLabel(tag, lang){
  ...
  if(tag==="starter") return "Start the sentence";
...
function praiseLine(lang){
  ...
  const bankEn = [
    ...
    "That’s a proper answer. Take the points.",
    "Good — clean and accurate. Move on.",
    "Yes. That’s the standard.",
    "Nice work — keep that level.",
    "Solid. You’re not guessing — you’re writing.",
    "That’s the kind of line that wins marks."
  ];
  const bankEs = [
    "Bien — eso puntúa.",
...
function tipForTags(tags, lang){
  ...
  if(lang==="es"){
    ...
    if(has("blank") || has("starter")) return "Start with ONE full sentence frame: \"Mi ____ es ____.\" or \"Me gusta ____ porque ____.\"";
...
const hasConnectorES = (s)=> /\b(y|pero|porque|además|entonces|también|sin\s+embargo|ya\s+que)\b/i.test(String(s||""));
  ...
  function unlockTargetForLevel(level){
    // Writing is slower than Turbo verbs — be generous early and tighten gradually.
    const lvl = Math.max(1, Math.min(10, Number(level)||1));
    // Level 1 ≈ 5:00, Level 10 ≈ 2:45 (time is still the tie-break motivator).
    return Math.max(165, 300 - (lvl-1) * 15);
  }

  // Wrongs allowed: Level 1 allows 4, taper down evenly to Level 10 allows 0 (mastery).
  function wrongsAllowed(level){
    // Keep it achievable: Level 1 allows 5 wrong, taper down to 1 by Level 10.
    const lvl = Math.max(1, Math.min(10, Number(level)||1));
    return Math.max(1, Math.round(5 - (lvl-1) * (4/9)));
  }
...
function jcMarking(promptText, answer, level, lang, rubric, det){
  ...
  // Score: reward intelligible communication first (buzz), then add small bonuses.
// Keep length as a minor factor, not the main driver.
let score = 6; // baseline: you wrote something meaningful

// If it's extremely short, cap the score (still some credit for attempting)
if(wc < 4) score = 3;

// Basic sentence/verb presence lifts it
const hasVerb = (lang==="es") ? /\b(soy|eres|es|somos|son|estoy|está|están|tengo|tiene|tenemos|tienen|me\s+gusta|me\s+encanta|voy|va|vamos|van|hago|hace|como|come|vivo|vive)\b/i.test(ans) : true;
if(wc >= 4 && hasVerb) score = 7;

// Bonus for meeting checklist items (modest)
const earnedBonus = earned.filter(l=>l!=="Length").length;
score += Math.min(2, earnedBonus); // +0..+2

// Very small bonus for fuller communication (never more than +1)
if(wc >= 10) score += 1;

// Language cogs reduce score (big ones matter most)
const tags = (det && det.tags) ? det.tags : [];
const hard = ["wrong_person","off_topic","verb_form","verb_ending","word_order","missing_be"];
...
// Hard grammar cogs should always be flagged (even if the sentence is long enough).
const hardBad = ["wrong_person","verb_form","verb_ending","agreement","missing_be","articles_gender","articles","word_order","off_topic"];
const hardHit = (det && Array.isArray(det.tags)) ? det.tags.find(t=>hardBad.includes(t)) : "";
if(ok && hardHit){
  ok = false;
  if(hardHit==="wrong_person") reason = "Wrong person (he/she vs you)";
  else if(hardHit==="verb_form" || hardHit==="verb_ending") reason = "Verb form";
  else if(hardHit==="agreement") reason = "Agreement";
  else if(hardHit==="missing_be") reason = "Missing ‘to be’";
  else if(hardHit==="articles_gender" || hardHit==="articles") reason = "Articles / gender";
  else if(hardHit==="word_order") reason = "Word order";
  else if(hardHit==="off_topic") reason = "Off-topic";
}
...
state.mark.items = state.mark.items.map(it=>{
  ...
  const task  = ref ? escapeHtml(ref.prompt||"") : "";
  ...
  const reasonMap = {
    ...
    missing_be: "You need the ‘to be’ verb to make descriptions work.",
    no_connector: "Your ideas need linking. One correct connector makes it flow.",
    too_short: "Too short = low marks. Add one detail and you jump up.",
    detail: "Detail wins marks. One extra fact is the difference."
  };
...
const pts = (Array.isArray(m.items) && m.items.length)
  ? m.items.reduce((a,it)=>a + (Number(it.score10)||0), 0)
  : 0;
const maxPts = (Array.isArray(m.items) ? m.items.length : 0) * 10;
  ...
  const html = `
    ...
    ${ (task || wrote || model) ? `
      <div class="muted" style="margin-top:8px">
        <b>Task:</b> ${task||"—"}<br>
        <b>Your line:</b> ${wrote||"—"}<br>
        <b>Example line (one way to score):</b> ${model||"—"}
      </div>` : "" }
    <div class="muted" style="margin-top:6px">
      Marks: <b>${pts}/${maxPts||50}</b> <span class="muted">(${pct}%)</span>
      <span class="muted"> • </span>
      Time: <b>${m.scoreSec.toFixed(1)}s</b>
      <span class="muted"> • </span>
      Wrong: <b>${m.wrong}</b>
      <span class="muted"> • </span>
...
function renderResults(){
    ...
    if(el.feedbackList){
      ...
      // Score summary (visible inside feedback, not just the top panel)
      try{
        const pts = (m.items||[]).reduce((a,it)=>a + (Number(it.score10)||0), 0);
        const maxPts = (m.items||[]).length * 10;
        const pct = maxPts ? Math.round((pts/maxPts)*100) : 0;
        const sum = document.createElement("div");
        sum.className = "fbSummary";
        sum.innerHTML = `<b>Total:</b> ${pts}/${maxPts} <span class="muted">(${pct}%)</span> <span class="muted">•</span> <b>Time:</b> ${m.scoreSec.toFixed(1)}s <span class="muted">•</span> <span class="muted">Each answer is marked /10 (verb + checklist + clarity). Time breaks ties.</span>`;
        el.feedbackList.appendChild(sum);
      }catch(_){ }
...
function gymFocusType(){
  ...
  if(tag==="starter" || tag==="blank") return "detail";
...
state.workshop.recentRepSigs = [];
  ...
  if(el.wsCogs){
    ...
    const top = Array.isArray(state.mark.topTags) ? state.mark.topTags : [];
    const also = top.filter(t=>t && t.tag && t.tag!==focusTag && t.tag!=="ok" && t.tag!=="detail").slice(0,3).map(t=>t.label);
    if(also.length){
...
if(false && (type==="detail" || type==="upgrade")){
...
<div class="wsHelpLine"><b>${escapeHtml(aim)}</b></div>`;
      ...
      if(type!=="detail" && type!=="upgrade") return;
...
if(type==="connector"){
      ...
      if(el.wsPrompt){
        const shown = (!base || base==="—" || isBadGymSeed(base)) ? "" : `
Your line: "${base}"`;
        el.wsPrompt.textContent = `Upgrade THIS line (add ONE extra detail).
Prompt: ${pr}${shown}`;
      }

      if(el.wsHelp){
        const esHints = [
          `Start with: "Mi ____ es ____."  Add: "porque ____."`,
          `Use: "En mi ____ hay ____ y ____."`,
          `Add ONE add-on: porque / y además / pero / también.`,
          `Keep it simple: sentence + ONE detail (where / why / with who).`
        ];
        const enHints = [
          `Write ONE clean sentence, then add ONE extra detail (because / and / but…).`,
          `Add ONE extra detail: where / why / how often / with who.`,
          `Keep it short: one sentence + one controlled add‑on.`,
          `Upgrade once: add "because…" or "and also…" — then stop.`
        ];
        const hint = (L==="es") ? esHints[Math.floor(Math.random()*esHints.length)]
                                : enHints[Math.floor(Math.random()*enHints.length)];
        el.wsHelp.textContent = `${hint}
Aim: ONE extra detail • 6+ words.`;
      }
    } else {
      if(el.wsPrompt) el.wsPrompt.textContent = "Type ONE improved sentence (clean + slightly longer).";
 el.wsPrompt.textContent = "Type ONE improved sentence (clean + slightly longer).";
