...
function promptTopicKey(promptText){
  ...
  if(/routine|school day|weekday|daily|horario|día escolar|un día/.test(t)) return "routine";
  if(/favo(u)?rite place|lugar favorito|sitio favorito/.test(t)) return "fav_place";
  if(/bedroom|habitaci[oó]n/.test(t)) return "bedroom";
  if(/classroom|aula|clase/.test(t)) return "classroom";
  if(/school|colegio|instituto|escuela/.test(t)) return "school";
  if(/best friend|friend|amig[oa]/.test(t)) return "friend";
  if(/family|familia/.test(t)) return "family";
  if(/favo(u)?rite subject|subject|asignatura/.test(t)) return "subject";
...
function esOneLineForTopic(promptText){
  ...
  switch(k){
    case "routine":
      return pick([
        "Normalmente me levanto a las siete y desayuno rápido.",
        "Me levanto temprano, voy al instituto y luego hago los deberes.",
        "Por la mañana tengo clases y por la tarde hago deporte."
      ]);
    case "food":
      return pick([
    ...
    case "classroom":
      return pick([
        ...
        "Mi clase es pequeña pero tenemos buena energía.",
    ...
    default:
      return pick([
        "Es divertido y me gusta bastante.",
        "Me gusta porque es interesante.",
        "Normalmente es tranquilo y agradable."
      ]);
  }
}
