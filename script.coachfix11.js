...
function presentCoachModal(){
  try{
    ...
    const also = Array.from(new Set(top.filter(t=>t && t.label && t.tag && t.tag!==focusTag).map(t=>t.label))).slice(0,3);
    ...
    const vibe = passed ? ((Math.random()<0.75) ? pick(COACH.praise) : pick(WIN_LINES)) : pick(LOSE_LINES);
    ...
    const model = (wrongItem && wrongItem.suggestion) ? String(wrongItem.suggestion) : pickModelAnswer({text: prompt});
...
function detectTags(prompt, answer, lang, rubric){
  ...
  if(lang==="es"){
    ...
    // --- Person mismatch (you/he/she) based on the prompt topic ---
    const topicK = promptTopicKey(prompt);
    if(/\b(eres|estás|tienes)\b/.test(low)){
      if(["friend","teacher","admire","celebrity","canteen","school","classroom","bedroom","fav_place","town","house","subject","sport","food","routine","after_school","family"].includes(topicK)){
        tags.push("wrong_person");
        examples.push("eres/estás/tienes → es/está/tiene (él/ella)");
      }
    }
    if(topicK==="appearance" && /^\s*(es|está)\b/.test(low) && !/^\s*mi\b/.test(low)){
      tags.push("wrong_person");
      examples.push("Es alto → Soy alto");
    }
...
function focusLabel(tag, lang){
  ...
  if(tag==="wrong_person") return "Verb person (tú vs él/ella)";
...
function tipForTags(tags, lang){
  ...
  if(lang==="es"){
    ...
    if(has("wrong_person")) return "Person mismatch: describing él/ella? Use es/está/tiene (not eres/estás/tienes).";
...
function praiseLine(lang){
  ...
  if(lang==="es") return pick([
    "That would score. Good.",
    "Yes. That reads well.",
    "Nice — clear and natural.",
    "Good work. Keep that standard."
  ]);
  if(lang==="fr") return pick([
    "Ça passe. Bien.",
    "Oui — c’est clair.",
    "Bien joué — c’est propre.",
    "Bon travail. Continue."
  ]);
  if(lang==="de") return pick([
    "Das passt. Gut.",
    "Ja — klar und sauber.",
    "Gut gemacht — weiter so.",
    "Das würde Punkte bringen."
  ]);
  return pick([
    "That would score. Good.",
    "Yes — that’s clear.",
    "Nice work. Keep it clean.",
...
function pickRoundFocus(items, lang, rubric){
  ...
  const labelMap = {
    ...
    wrong_person: "Verb person (tú vs él/ella)",
...
function buildSuggestionForItem(prompt, answer, lang, rubric, focusTag){
  ...
  if(!aRaw || isBadGymSeed(aRaw)) {
    return (lang==="es") ? esOneLineForTopic(pRaw) : "—";
  }
...
const hasPorqueES  = (s)=> {
  const x = String(s||"");
  if(/\b(porque|ya\s+que|debido\s+a|gracias\s+a)\b/i.test(x)) return true;
  // Accept "por + noun/possessive" as a reason, but ignore time phrases like "por la mañana"
  if(/\bpor\s+la\s+(mañana|tarde|noche)\b/i.test(x)) return false;
  if(/\bpor\s+(mi|tu|su|el|la|los|las|un|una|mis|tus|sus)\b/i.test(x)) return true;
  if(/\bpor\s+su\s+\w+/i.test(x)) return true;
  return false;
};
...
// --- Coach Gym helpers (Spanish) ---
function promptTopicKey(promptText){
  const t = String(promptText||"").toLowerCase();

  // Most specific first
  if(/appearance|look\s+like|describe\s+yourself|how\s+you\s+look|aspecto|\bc[oó]mo\s+eres\b/.test(t)) return "appearance";
  if(/strengths|weaknesses|strong\s+points|weak\s+points|puntos\s+fuertes|debilidades/.test(t)) return "strengths";
  if(/person\s+you\s+admire|admire|admiro|role\s+model|modelo\s+a\s+seguir/.test(t)) return "admire";
  if(/teacher|profesor|profesora|maestro|maestra/.test(t)) return "teacher";
  if(/celebrity|famous|famoso|famosa|celebridad/.test(t)) return "celebrity";

  if(/routine|school\s*day|weekday|daily|horario|d[ií]a\s+escolar|un\s+d[ií]a/.test(t)) return "routine";
  if(/what\s+you\s+do\s+after\s+school|after\s+school|despu[eé]s\s+de\s+clase/.test(t)) return "after_school";

  if(/eat|food|meal|drink|comer|comida|beb(er|ida)|desayun|almuerz|cena/.test(t)) return "food";

  if(/favo(u)?rite\s+place|lugar\s+favorito|sitio\s+favorito/.test(t)) return "fav_place";
  if(/bedroom|habitaci[oó]n|dormitorio/.test(t)) return "bedroom";
  if(/classroom|aula|clase/.test(t)) return "classroom";
  // School places (more specific than "school")
  if(/canteen|cantina|comedor|cafeter[ií]a/.test(t)) return "canteen";
  if(/school|colegio|instituto|escuela/.test(t)) return "school";

  if(/best\s+friend|friend|amig[oa]/.test(t)) return "friend";
  if(/family|familia/.test(t)) return "family";

  if(/favo(u)?rite\s+subject|subject|asignatura/.test(t)) return "subject";
  if(/favo(u)?rite\s+sport|sport|deporte/.test(t)) return "sport";

  if(/town|city|village|pueblo|ciudad|barrio/.test(t)) return "town";
  if(/phone|telephone|tel[eé]fono|m[oó]vil|mobile/.test(t)) return "phone";
  if(/house|home|casa/.test(t)) return "house";
  return "generic";
}


function gymStartersEN_ES(promptText){
  const k = promptTopicKey(promptText);
  const wrap = (a,b,c)=> ({a,b,c});
  switch(k){
    case "routine":
      return wrap('Start with: “Normalmente ____.”','Add: “Luego ____.”','Add: “Después ____.”');
    case "appearance":
      return wrap('Start with: “Soy ____.”','Add: “Tengo ____.”','Add: “Mis ojos son ____.”');
    case "teacher":
      return wrap('Start with: “Mi profesor/a de ____ es ____.”','Add: “Explica ____.”','Add: “Me cae bien porque ____.”');
    case "admire":
      return wrap('Start with: “Admiro a ____.”','Add: “por su ____.”','Add: “porque ____.”');
    case "celebrity":
      return wrap('Start with: “Mi celebridad favorita es ____.”','Add: “Es ____.”','Add: “porque ____.”');
    case "strengths":
      return wrap('Start with: “Soy ____.”','Add: “pero a veces ____.”','Add: “En general ____.”');
    case "after_school":
      return wrap('Start with: “Después de clase ____.”','Add: “Luego ____.”','Add: “También ____.”');
    case "food":
      return wrap('Start with: “Me gusta(n) ____.”','Add: “porque ____.”','Add: “Normalmente como ____.”');
    case "friend":
      return wrap('Start with: “Mi mejor amigo/a es ____.”','Add: “También es ____.”','Add: “porque ____.”');
    case "family":
      return wrap('Start with: “En mi familia somos ____.”','Add: “Tengo ____.”','Add: “y ____.”');
    case "fav_place":
      return wrap('Start with: “Mi lugar favorito es ____.”','Add: “porque ____.”','Add: “y además ____.”');
    case "bedroom":
      return wrap('Start with: “Mi habitación es ____.”','Add: “Tengo ____.”','Add: “y ____.”');
    case "classroom":
      return wrap('Start with: “Mi aula es ____.”','Add: “Hay ____.”','Add: “y ____.”');
    case "canteen":
      return wrap('Start with: “En la cantina/comedor ____.”','Add: “Hay ____.”','Add: “y ____.”');
    case "school":
      return wrap('Start with: “Mi instituto es ____.”','Add: “Tiene ____.”','Add: “y ____.”');
    case "subject":
      return wrap('Start with: “Me gusta ____.”','Add: “porque ____.”','Add: “y además ____.”');
    case "town":
      return wrap('Start with: “Vivo en ____.”','Add: “Hay ____.”','Add: “y ____.”');
    case "phone":
      return wrap('Start with: “Mi móvil es ____.”','Add: “Lo uso para ____.”','Add: “y ____.”');
    default:
      return wrap('Start with: “Es ____.”','Add: “y ____.”','Add: “porque ____.”');
  }
}

function esOneLineForTopic(promptText){
  const k = promptTopicKey(promptText);
  const pick = (arr)=> arr[Math.floor(Math.random()*arr.length)];
  ...
  switch(k){
    case "appearance":
      return pick([
        "Soy alto y delgado y tengo el pelo corto y castaño.",
        "Soy bastante bajo y tengo los ojos marrones y el pelo rizado.",
        "Tengo el pelo largo y soy moreno; normalmente visto ropa deportiva."
      ]);

    case "teacher":
      return pick([
        "Mi profesor de español es simpático y explica muy bien.",
        "Mi profesora es exigente pero justa, y las clases son divertidas.",
        "Mi profe es paciente y nos ayuda cuando no entendemos."
      ]);

    case "admire":
      return pick([
        "Admiro a mi madre porque trabaja mucho y nunca se rinde.",
        "Admiro a Diego Simeone por su liderazgo y su perseverancia.",
        "Admiro a una persona de mi familia porque siempre ayuda a los demás."
      ]);

    case "celebrity":
      return pick([
        "Mi celebridad favorita es divertida y me inspira mucho.",
        "Me gusta esta celebridad porque es humilde y muy talentosa.",
        "Mi famoso favorito es trabajador y siempre da lo mejor."
      ]);

    case "strengths":
      return pick([
        "Soy creativo pero a veces soy un poco desorganizado.",
        "Soy responsable y trabajador, aunque me pongo nervioso a veces.",
        "Se me da bien hablar con la gente, pero me cuesta concentrarme."
      ]);

    case "routine":
      return pick([
        "Normalmente me levanto a las siete, desayuno y voy al instituto.",
        "Por la mañana tengo clases y por la tarde hago los deberes.",
        "Me levanto temprano, voy a clase y luego juego al fútbol."
      ]);

    case "after_school":
      return pick([
        "Después de clase hago deporte y luego estudio un poco.",
        "Al salir del instituto quedo con mis amigos y hago los deberes.",
        "Después del cole voy a casa, meriendo y estudio."
      ]);

    case "food":
      return pick([
        "Me gusta comer patatas y plátanos porque son ricos.",
        "Normalmente como pasta y fruta, y bebo agua.",
        "Me encanta la pizza porque está muy buena."
      ]);

    case "friend":
      return pick([
        "Mi mejor amigo es alto y muy divertido.",
        "Mi mejor amiga es simpática y siempre ayuda a los demás.",
        "Mi mejor amigo es inteligente y le gusta el deporte."
      ]);

    case "family":
      return pick([
        "En mi familia somos cinco y nos llevamos bastante bien.",
        "Tengo un hermano y una hermana, y mi perro es muy juguetón.",
        "En mi casa vivimos cuatro y cenamos juntos a veces."
      ]);

    case "fav_place":
      return pick([
        "Mi lugar favorito de la casa es mi habitación porque es tranquila.",
        "Mi sitio favorito es el salón porque es cómodo y vemos la tele.",
        "Me gusta la cocina porque huele bien y hablamos en familia."
      ]);

    case "bedroom":
      return pick([
        "Mi habitación es pequeña pero cómoda y tengo un escritorio.",
        "En mi habitación hay una cama grande y un armario.",
        "Mi dormitorio es tranquilo y tengo pósters en la pared."
      ]);

    case "classroom":
      return pick([
        "Mi aula es luminosa y hay pósters en las paredes.",
        "Mi clase es pequeña pero tenemos buen ambiente.",
        "En el aula hay una pizarra grande y muchas mesas."
      ]);

    case "canteen":
      return pick([
        "En la cantina del instituto hay bocadillos y la comida está bastante buena.",
        "El comedor es pequeño pero limpio y casi siempre hay cola.",
        "En la cafetería hay sándwiches y fruta, y el ambiente es bueno."
      ]);

    case "school":
      return pick([
        "Mi instituto es moderno y los profesores son justos.",
        "Mi colegio es grande y tiene un patio para jugar.",
        "En mi escuela hay muchas actividades después de clase."
      ]);

    case "subject":
      return pick([
        "Me gusta el español porque lo entiendo y me resulta fácil.",
        "Mi asignatura favorita es historia porque es interesante.",
        "Me encanta educación física porque hago deporte con mis amigos."
      ]);

    case "sport":
      return pick([
        "Mi deporte favorito es el fútbol porque es emocionante.",
        "Me gusta el baloncesto porque juego con mis amigos.",
        "Prefiero nadar porque me relaja y es saludable."
      ]);

    case "town":
      return pick([
        "Vivo en un pueblo pequeño y hay un parque donde quedo con mis amigos.",
        "Mi barrio es tranquilo y hay tiendas y cafeterías cerca.",
        "En mi ciudad hay un cine y me gusta ir los fines de semana."
      ]);

    case "phone":
      return pick([
        "Mi móvil es negro y lo uso para hablar con mis amigos.",
        "Tengo un teléfono bastante rápido y hago fotos a veces.",
        "Uso el móvil para mandar mensajes y escuchar música."
      ]);

    case "house":
      return pick([
        "Mi casa es bastante moderna y tiene una cocina grande.",
        "En mi casa hay un jardín pequeño y dos baños.",
        "Mi casa es cómoda y me gusta estar en el salón."
      ]);

    default:
      return pick([
        "Está bien: una frase limpia y un detalle más.",
        "Bien: ahora añade un motivo (porque...) y subes rápido.",
        "Correcto: una idea clara y una conexión (y/pero/porque)."
      ]);
  }
}


function gymDetailChoiceOptionsES(promptText){
  const k = promptTopicKey(promptText);
  const pick = (arr)=> arr[Math.floor(Math.random()*arr.length)];
  let base, good;
  switch(k){
    case "routine":
      base = pick(["Normalmente me levanto temprano.","Por la mañana voy al instituto.","Después de clase hago deporte."]);
      good = base.replace(/\.$/,"") + " y luego hago los deberes.";
      break;
        case "appearance":
      base = pick(["Soy alto.","Soy bastante bajo.","Tengo el pelo corto."]);
      good = pick([
        "Soy alto y tengo los ojos azules.",
        "Soy bastante bajo y tengo el pelo rizado.",
        "Tengo el pelo corto y soy moreno."
      ]);
      break;
    case "teacher":
      base = pick(["Mi profesor es simpático.","Mi profesora es justa.","Mi profe explica bien."]);
      good = pick([
        "Mi profesor de español es simpático y explica muy bien.",
        "Mi profesora es exigente pero justa, y las clases son divertidas.",
        "Mi profe es paciente y nos ayuda cuando no entendemos."
      ...
      break;
    case "admire":
      base = pick(["Admiro a mi madre.","Admiro a Diego Simeone.","Admiro a mi hermano."]);
      good = pick([
        "Admiro a mi madre porque trabaja mucho y nunca se rinde.",
        "Admiro a Diego Simeone por su liderazgo y su perseverancia.",
        "Admiro a una persona de mi familia porque siempre ayuda a los demás."
      ...
      break;
    case "celebrity":
      base = pick(["Mi celebridad favorita es famosa.","Me gusta esta celebridad.","Mi famoso favorito es muy bueno."]);
      good = pick([
        "Mi celebridad favorita es divertida y me inspira mucho.",
        "Me gusta esta celebridad porque es humilde y muy talentosa.",
        "Mi famoso favorito es trabajador y siempre da lo mejor."
      ...
      break;
    case "strengths":
      base = pick(["Soy creativo.","Soy responsable.","Soy trabajador."]);
      good = pick([
        "Soy creativo pero a veces soy un poco desorganizado.",
        "Soy responsable y trabajador, aunque me pongo nervioso a veces.",
        "Se me da bien hablar con la gente, pero me cuesta concentrarme."
      ...
      break;
    case "after_school":
      base = pick(["Después de clase voy a casa.","Al salir del instituto meriendo.","Después del cole descanso un poco."]);
      good = pick([
        "Después de clase hago deporte y luego estudio un poco.",
        "Al salir del instituto quedo con mis amigos y hago los deberes.",
        "Después del cole voy a casa, meriendo y estudio."
      ...
      break;
case "food":
      base = pick(["Me gusta comer patatas.","Me gusta la fruta.","Normalmente como pasta."]);
      good = base.replace(/\.$/,"") + " porque es rica.";
      break;
    case "friend":
      base = pick(["Mi mejor amigo es alto.","Mi mejor amiga es simpática.","Mi mejor amigo es divertido."]);
      good = base.replace(/\.$/,"") + " y siempre ayuda a los demás.";
      break;
    case "family":
      base = pick(["En mi familia somos cuatro.","Tengo un hermano.","Vivo con mis padres."]);
      good = base.replace(/\.$/,"") + " y nos llevamos bien.";
      break;
    case "fav_place":
      base = pick(["Mi lugar favorito es mi habitación.","Mi sitio favorito es el salón.","Mi lugar favorito es la cocina."]);
      good = base.replace(/\.$/,"") + " porque es muy cómodo.";
      break;
    case "bedroom":
      base = pick(["Mi habitación es pequeña.","En mi habitación hay una cama.","Tengo un escritorio."]);
      good = base.replace(/\.$/,"") + " y un armario grande.";
      break;
    case "classroom":
      base = pick(["Mi aula es luminosa.","En mi clase hay muchas mesas.","La clase es pequeña."]);
      good = base.replace(/\.$/,"") + " y hay pósters en la pared.";
      break;
    case "canteen":
      base = pick([
        "En la cantina hay bocadillos.",
        "El comedor es pequeño.",
        "En la cafetería hay mucha gente."
      ]);
      good = pick([
        "En la cantina del instituto hay bocadillos y la comida está bastante buena.",
        "El comedor es pequeño pero limpio y casi siempre hay cola.",
      ...
      break;
    case "school":
      base = pick(["Mi instituto es grande.","El colegio es moderno.","Mi escuela es buena."]);
      good = base.replace(/\.$/,"") + " y tiene buenas instalaciones.";
      break;
    case "subject":
      base = pick(["Me gusta el español.","Me gusta historia.","Me encanta educación física."]);
      good = base.replace(/\.$/,"") + " porque es interesante.";
      break;
    case "town":
      base = pick(["Vivo en un pueblo pequeño.","Mi barrio es tranquilo.","Mi ciudad es grande."]);
      good = base.replace(/\.$/,"") + " y hay un parque cerca.";
      break;
    default:
      base = "Es bueno.";
      good = "Es bueno y además es interesante para mí.";
  }
  const weak = (k==="school") ? "Es bueno." : "Es bueno.";
  const distract1 = esOneLineForTopic("Describe your best friend."); // safe distractor
  const opts = shuffle([good, base, weak, distract1]).slice(0,4);
  return {opts, correct: good};
}
...
const adv = tipForTags(det.tags, state.lang);
const t2 = ok ? (adv ? (praiseLine(state.lang)+" "+adv) : praiseLine(state.lang)) : (adv || tip);
...
function gymFocusType(){
  ...
  if(tag==="wrong_person") return "verbs";
...
if(L==="es"){
  const pack = gymDetailChoiceOptionsES(ptxt||"");
  opts = pack.opts || [];
...
opts.forEach(o=>{
  ...
  b.className="wsChoice"; b.type="button"; b.textContent=o;
  b.addEventListener("click", ()=>{
    const ok = (L==="es") ? (o===correct) : ((o===correct) || o.startsWith(correct.slice(0, Math.min(18, correct.length))));
...
if(el.wsHelp){
  ...
  if(L==="es"){
    const st = gymStartersEN_ES(ptxt||"");
    el.wsHelp.innerHTML = `
      <div class="wsHelpLine"><b>PROMPT:</b> ${escapeHtml(ptxt||"—")}</div>
      <div class="wsHelpLine"><b>STARTERS:</b> ${escapeHtml(st.a)} / ${escapeHtml(st.b)}${st.c?(" / "+escapeHtml(st.c)):""}</div>
...
"${basePrompt||"die Aufgabe"}"`;
      ...
      if(el.wsHelp){
        ...
        if(L==="es"){ const st = gymStartersEN_ES(basePrompt||""); lines.push(`Starters: ${st.a} / ${st.b}${st.c?(" / "+st.c):""}`); }
